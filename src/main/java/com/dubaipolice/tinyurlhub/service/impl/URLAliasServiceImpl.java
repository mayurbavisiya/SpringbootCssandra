...
import com.dubaipolice.tinyurlhub.util.URLAliasServiceUtil;
...
public class URLAliasServiceImpl implements URLAliasService {
	...
	public CompletableFuture<URLAliasMappingEntity> createAlias(URLAliasMappingDTO dto) throws Exception {
		...
		URLAliasServiceUtil.validateRequest(dto);
		...
		if (!list.isEmpty()) {
			...
			list.get(0).setAlias(Constants.urlConstantDomain + list.get(0).getAlias());
			...
			enity.setDate(date);
			enity.setAlias(Constants.urlConstantDomain + enity.getAlias());
			return CompletableFuture.completedFuture(enity);
		}
	...
	public CompletableFuture<String> getRealURL(String alias) throws Exception {
		...
		ExceptionUtil.throwNullOrEmptyValidationException("alias", alias, false);
		List<URLAliasMappingEntity> list = new ArrayList<URLAliasMappingEntity>();
		...
		if (!list.isEmpty()) {
			...
			if (!Utils.isLinkExpired(createdDate, expiryDays)) {
				if (list.get(0).getNoOfHit() < list.get(0).getUrlAccessLimit()) {
					...
					//Pointer reach here means its valid and not expired url. Hence we will increament counter
					list.get(0).setNoOfHit(list.get(0).getNoOfHit() + 1);					
					repository.save(list.get(0));
					
					list.get(0).setDate(Utils.getDateFromUUID(list.get(0).getCreated_date(), Constants.ddmmyyyyHHmmss));//Changing date format in readable format					
